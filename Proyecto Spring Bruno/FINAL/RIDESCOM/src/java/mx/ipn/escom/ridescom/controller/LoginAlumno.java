                    ...
                    String fail=f.text();
                            ...
                            if(fail.isEmpty()) {
                                //Aquí se escanea el comprobante del alumno que inició sesión
                                    ...
                                    String grupo=e.text();
                                    if(grupo.matches("")){
                                        mav.addObject("mjs", "<div style='color: #ffffff;'>LO SENTIMOS NO ESTÁS INSCRITO, VUELVE CUANDO LO ESTÉS</div>"); 
                                        mav.setViewName("LoginAlumno");
                                    }else {
                                            //Checo si ya tengo usuario y boleta guardado
                                        String ud="select ID_Alumno from persona p inner join (alumno a, usuario u) on (p.ID_Persona=a.Persona_ID_Persona and p.ID_Persona=u.Persona_ID_Persona) where ID_Alumno='"+usuario+"'and Nombre_U='"+usuario+"'";
                                        try{
                                           cta=cn.Connect();
...
//                                                mav.setViewName("redirect:/Alumno.html");
                                            ...
                                            dtn1="insert into Contacto(Persona_ID_Persona) values((select MAX(ID_Persona)from Persona))";
                                            this.rid.update(dtn1);
                                            dtn2="insert into Telefono_Celular (Celular, Contacto_ID_Contacto) values ('5555555555',(select MAX(ID_Contacto)from Contacto))";
                                            this.rid.update(dtn2);
                                            dtn3="insert into Email (Correo, Contacto_ID_Contacto) values ('tucorreo@hotmail.com',(select MAX(ID_Contacto)from Contacto))";
                                            this.rid.update(dtn3);
                                            dtn4="insert into Telefono_fijo (Telefono, Contacto_ID_Contacto) values ('55555555',(select MAX(ID_Contacto)from Contacto))";
                                            this.rid.update(dtn4);
                                            ...
                                            regis="insert into Usuario (Nombre_U, Activo, Fecha_inicio, Roles_ID_Roles, Persona_ID_Persona) values("+usuario+",1,left(now(),10),3,(select MAX(ID_Persona)from Persona))";
                                            this.rid.update(regis);
...
//                                            mav.setViewName("redirect:/InfoAlumno.html");
                    ...
                    mav.addObject("mjs", "<div style='color: red;'>Usuario, contraseña y/o clave captcha incorrectos intente nuevamente </div>"); 
                    mav.setViewName("redirect:/LoginAlumno.html");
                }
        }
        }else{
