	...
	<section id="global">
		...
		<div class="col-12 ">
			<div id="noti" class="alert alert-info col-12" role="alert">
			  <p> Aquí podrás dar de alta un nuevo evento interpolitécnico deportivo. </p>
			  <p> Recuerda verificar todos los datos antes de concluir. </p>
			  <p> Su evento unicamente puede ser registrado 5 días después del día en que se crea. </p>
			</div>
		</div>
                ...
                <form class="needs-validation" novalidate method="POST">
                    <div class="form-row">
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <input name="FE" type="text" class="fec form-control" id="datepicker" placeholder="YYYY-MM-DD" maxlength="10" autocomplete="off" required/>
                            <div class="invalid-feedback">
                                Ingresa una fecha válida
                            </div>
                            <script>
                                var today, datepicker;
                                today = new Date(new Date().getFullYear(), new Date().getMonth(), new Date().getDate()+5);
					  ...
					  <option value="">Ciclo escolar</option>
                                                    <%
                                                        try{
                                                            String query="select MAX(ID_Ciclo)as ID, MAX(Ciclo_Escolar)as CicloE from Ciclo";
                                                            ...
                                                            while(rs.next()){
                                                                %>
                                                                <option value="<%=rs.getInt("ID")%>"><%=rs.getString("CicloE")%></option>
                                                                <%
                                                            }
...
<!--                            <input name="ciclo" type="text" class="form-control" id="validationTooltip02" placeholder="YYYY/1-2" disabled>-->
                    ...
                    <div>
					  ...
					  <option value="">Seleccione prueba...</option>
                                                    <%
                                                        try{
                                                            String query="select ID_Pruebas, Prueba, Disciplina, Rama from pruebas pr, Act_Deportiva d, Tipo_Pruebas tp where pr.Act_Deportiva_ID_Deporte=d.ID_Deporte and tp.ID_Tipo=pr.Tipo_Pruebas_ID_Tipo order by Disciplina,Prueba ASC";
                                                            Class.forName("com.mysql.jdbc.Driver").newInstance();
                                                            Connect conn = new Connect();
                                                            Statement stm=conn.Connect().createStatement();
                                                            ResultSet rs=stm.executeQuery(query);
                                                            while(rs.next()){
                                                                %>
                                                                <option value="<%=rs.getInt("ID_Pruebas")%>"><%=rs.getString("Prueba")%>-<%=rs.getString("Disciplina")%>-<%=rs.getString("Rama")%></option>
                                                                <%
                                                            }
